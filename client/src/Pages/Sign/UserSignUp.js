...
if(password.length > min||password.length < max){
  ...
  if(regPassword.test(password)){
    setPassErrorMessage('영문,특수문자(1개이상),숫자(1개이상) 조합하여 입력해주세요')
  }
   else{
    setPassErrorMessage('')
   } 
...
{
  axios.post('http://localhost:8080/auth/nick',{
    nickname:nickname
  },{headers:{
 ...
 //최종 회원가입을 눌렀을때 
  ...
  if(validEmail&&validPassword){
    ...
    axios.post('http://localhost:8080/auth/signup',{
      ...
      want_vol:want_vol,want_region:want_region,age:age,sex:sex
    ...
    <>
      <Wrapper>
        ...
        <SelectBox>
          ...
          <AgeButton  onClick={handleTeen}
        ref={teenInput}>
            청소년
          </AgeButton >
          <AgeButton  onClick={handleAdult}
        ref={adultInput}>
            청년
          </AgeButton>
          <AgeButton  onClick={handleSenior}
        ref={seniorInput}>
            장년
          </AgeButton>
        </SelectBox>
        <CompleteBox>
          <CompleteButton onClick={handleSignUpRequest}>회원가입 완료</CompleteButton>
        </CompleteBox>
      </Wrapper>
    </>
  );
