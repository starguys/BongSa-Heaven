...
module.exports = {
  ...
  editControl: async (req, res) => {
    ...
    // 3. 업데이트
    const {email, password, nickname, sex, want_region, want_vol, age, company, iscompany, isopen} = req.body;
    ...
    if (password.length > 30) {
      ...
      crypto.randomBytes(64, (err, buf) => {
        if (err) {
          ...
          crypto.pbkdf2(password, newSalt, 110011, 64, 'sha512', async (err, key) => {
            if (err) {
              console.log(err);
              return;
            } else {
  ...
  passwordControl: async (req, res) => {
    ...
    if (userData) {
      ...
      crypto.pbkdf2(checkPassword, salt.salt, 110011, 64, 'sha512', async (err, key) => {
